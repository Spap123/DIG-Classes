...
lag(emp_salary,1,0) over(partition by emp_department order by emp_id) as previous_salary,
case
	when e.emp_salary > lag(emp_salary,1,0) over(partition by emp_department order by emp_id) then 'Higher'
    when e.emp_salary < lag(emp_salary,1,0) over(partition by emp_department order by emp_id) then 'Lower'
    when e.emp_salary = lag(emp_salary,1,0) over(partition by emp_department order by emp_id) then 'Same'
    end as salary_range
from details e;
